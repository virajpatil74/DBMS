...
def customer_update(Cemail, Cnumber):
    c.execute(
        ''' UPDATE Customers SET C_Number = ? WHERE C_Email = ?''', (Cnumber, Cemail,))
    conn.commit()
    print("Updating")
    ...
    if choice == "Drugs":
        ...
        if choice == "Add":
            ...
            if st.button("Add Drug"):
                drug_add_data(drug_name, drug_expiry,
                              drug_mainuse, drug_quantity, drug_id)
        ...
        if choice == "View":
            ...
            with st.expander("View All Drug Data"):
                drug_clean_df = pd.DataFrame(
                    drug_result, columns=["Name", "Expiry Date", "Use", "Quantity", "ID"])
        ...
        if choice == "View":
            ...
            with st.expander("View All Order Data"):
                order_clean_df = pd.DataFrame(
                    order_result, columns=["Name", "Items", "Qty", "ID"])
                st.dataframe(order_clean_df)
    elif choice == "About":
        st.subheader("DBMS Mini Project")
        st.subheader("Made By Viraj, Amey and Tanay ")
...
def customer(username, password):
    if getauthenicate(username, password):
        ...
        dolo650 = st.slider(label="Quantity", min_value=0,
                            max_value=500, key=1)
        ...
        strepsils = st.slider(
            label="Quantity", min_value=0, max_value=200, key=2)
        ...
        vicks = st.slider(label="Quantity", min_value=0, max_value=50, key=3)
        ...
        if st.button(label="Buy now"):
            ...
            if int(vicks) > 0:
                O_items += "Vicks"
            O_Qty = str(dolo650)+str(',') + \
                str(strepsils) + str(",") + str(vicks)
    ...
    if choice == "Login":
        ...
        if st.button("Signup"):
            if (cust_password == cust_password1):
                customer_add_data(cust_name, cust_password,
                                  cust_email, cust_area, cust_number,)
